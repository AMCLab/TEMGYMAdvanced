...
class AnalyticalLaplace():
    ...
    def MagPotentialCyl(self, order = 1):
    ...
    def MagPotentialCart(self, order = 1):
    ...
    def SchiskeField(self, a = 0.025, phi_0 = 30e3, k = 0.5**(1/2)):
        z = self.z
    ...
    def SchiskeField_(self, a = 0.025, phi_0 = 30e3, k = 0.5**(1/2)):
        z = self.z
        phi = -phi_0*((k**2)/(1+(z/a)**2))
        
        return phi
    
    def KormskaBiprism(self, Vf = 90, r = 0.25e-6, R = 1e-3, z_offset = 0):
        z = self.z
        x = self.x
        phi = Vf*(sp.log((x*x+(z-z_offset)**2)/(R**2)))/(2*sp.log(r/R))
    ...
    def SeptierBiprism(self, r = 1e-5, R = 1e-2, z_offset = 0):
        z = self.z
        x = self.x
        Vf = self.V
        phi = Vf/(2*sp.log(((np.pi*r)/(4*R))))*(
                    sp.log((sp.cosh((np.pi*(z-z_offset))/(2*R))-sp.cos((np.pi*x)/(2*R)))/
                            (sp.cosh((np.pi*(z-z_offset))/(2*R))+sp.cos((np.pi*x)/(2*R)))))
    ...
    def RoundLensFieldCartE(self, U):
        ...
        U_lambda = sp.lambdify((x, y, z), (potential), 'math')
